...
def send_admin_login_approval(*, approve_url: str, username: str, client_ip: str) -> bool:
    """Notify admin login attempt; message includes one-time approve link."""
    if not _is_configured():
        logger.warning("Telegram not configured for admin login approval")
        return False

    token = settings.TELEGRAM_BOT_TOKEN
    chat_id = settings.TELEGRAM_CHAT_ID
    safe_user = escape(username or "—")
    safe_ip = escape(client_ip or "unknown")
    safe_url = escape(approve_url or "")

    html_text = (
        "🔐 <b>Вход в админку — подтвердите</b>\n\n"
        f"👤 <b>Пользователь:</b> {safe_user}\n"
        f"🌐 <b>IP:</b> {safe_ip}\n\n"
        f"🔗 <a href=\"{safe_url}\">Подтвердить вход</a>\n\n"
        "Ссылка действует 20 минут."
    )
    plain_text = (
        "Вход в админку — подтвердите\n\n"
        f"Пользователь: {username or '—'}\n"
        f"IP: {client_ip or 'unknown'}\n\n"
        f"Подтвердить: {approve_url}\n\n"
        "Ссылка действует 20 минут."
    )
    ...
    for text, parse_mode in ((html_text, "HTML"), (plain_text, None)):
        ...
        if ok:
            return True
        logger.error(
            "Telegram admin approval failed (parse_mode=%s): %s",
            parse_mode or "plain",
            _redact_secrets(detail, token),
        )
    return False
